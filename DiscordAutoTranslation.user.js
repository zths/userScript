...
(function() {
    ...
    unsafeWindow.hookAjax({
        ...
        send: function(args,xhr){
            if(!unsafeWindow.GMDiscordTrsOn){
                return false;
            }
                        ...
                        if(msg.content && msg.content.length > 1){
                            var cb = (function(msg,xhr){
                                return function(text){
                                    if(unsafeWindow.GMDiscordTrsOnlyDist && msg.content){
                                        msg.content = text;
                                    }else{
                                        if(msg.content !== text && msg.content){
                                            msg.content = msg.content + unsafeWindow.GMDiscordTrssplitStrDef + text;
                                        }
                                    }
                                    xhr.send(JSON.stringify(msg));
                                }
                            })(msg,xhr);
                            window["apiCallBackText"+unsafeWindow.GMDiscordTrsEngine](msg.content,cb);
                            return true;
                        }
                    }catch(e){
                        console.error(e);
        ...
        GM_xmlhttpRequest({
            ...
            onload:     function (responseDetails) {
                ...
                try{
                    ...
                    var tt = "";
                    result[0].forEach(function(a){tt += a[0]});
                    callback(tt);
                }catch(e){
                    callback("");
                }
    ...
    window.apiCallBackTextBaidu = function (text,callback){
        callback(text);
    }
    ...
    window.apiCallBackTextCaiyun = function(text,callback){
        GM_xmlhttpRequest({
            ...
            "headers": {
                ...
                'x-authorization': "token " + unsafeWindow.GMDiscordTrsApiKey
            },
            onerror: function(error){
                console.error(error);
            }
    ...
    function toggleOptions(){
        ...
        inputTrs.checked = unsafeWindow.GMDiscordTrsOn;
        inputTrs.onchange = function(){
        ...
        inputOnlyTrs.type="checkbox";
        inputOnlyTrs.checked = unsafeWindow.GMDiscordTrsOnlyDist;
        inputOnlyTrs.onchange = function(){
        ...
        inputTrsSplit.type="text";
        var GMDiscordTrssplitStrDefify = JSON.stringify(unsafeWindow.GMDiscordTrssplitStrDef);

        inputTrsSplit.value = GMDiscordTrssplitStrDefify.substr(1, GMDiscordTrssplitStrDefify.length-2);
        inputTrsSplit.onchange = function(){
            var val = JSON.parse('"'+inputTrsSplit.value.replace(/"/g,'\"')+'"');
            unsafeWindow.GMDiscordTrssplitStrDef = val;
            GM_setValue('GMDiscordTrssplitStrDef',val);
        };
        inputTrsSplit.onkeydown = function(){
            var val = JSON.parse('"'+inputTrsSplit.value.replace(/"/g,'\"')+'"');
            unsafeWindow.GMDiscordTrssplitStrDef = val;
            GM_setValue('GMDiscordTrssplitStrDef',val);
        };
        ...
        inputTrsApiKey.type="text";
        inputTrsApiKey.value = unsafeWindow.GMDiscordTrsApiKey;
        inputTrsApiKey.onchange = function(){
            unsafeWindow.GMDiscordTrsApiKey = inputTrsApiKey.value;
            GM_setValue('GMDiscordTrsApiKey',inputTrsApiKey.value);
        };
        inputTrsApiKey.onkeydown = function(){
            unsafeWindow.GMDiscordTrsApiKey = inputTrsApiKey.value;
            GM_setValue('GMDiscordTrsApiKey',inputTrsApiKey.value);
        };
